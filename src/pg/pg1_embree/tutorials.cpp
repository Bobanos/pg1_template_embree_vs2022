...
int tutorial_2()
{
	// create texture
	printf("LDR texture\n");
	Texture texture_ldr( "../../../data/test.png" );
	for ( int x = 0; x < 7; ++x )
	{
		Color3f texel = texture_ldr.get_texel( x, 0 );
	...
	(r = 0.498, g = 0.498, b = 0.498)
	(r = 1.000, g = 0.000, b = 1.000)

	Note that the actual values may be slightly off due to lossy compression used in particular image formats.
	Also note that the gamma compression may affect actual values considerably.
	*/

	printf( "\nHDR texture\n" );
	Texture texture_hdr( "../../../data/test.exr" );
	for ( int x = 0; x < 7; ++x )
	{
		Color3f texel = texture_hdr.get_texel( x, 0 );
		printf( "(r = %0.3f, g = %0.3f, b = %0.3f)\n", texel.r, texel.g, texel.b );
	}
...
int tutorial_3( const std::string file_name, const char * config )
{
	...
	raytracer.LoadBackground( "../../../data/kloofendal_43d_clear_2k.hdr" );
